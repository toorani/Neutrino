...
{
    public class SellerPromotionBS : NeutrinoBusinessService, ISellerPromotionBS
    {
        ...
        public async Task<IBusinessResultValue<decimal>> LoadTotalPromotionAsync(int memberId, int month, int year)
        {
            ...
            {
                // پورسانت عوامل فروش
                result.ResultValue = await (from mep in unitOfWork.SellerPromotionDataService.GetQuery()
